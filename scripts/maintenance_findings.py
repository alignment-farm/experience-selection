...
p=argparse.ArgumentParser();p.add_argument('--metrics',type=Path,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--figure',default='');a=p.parse_args()
m=json.loads(a.metrics.read_text());arms=m['arms'];fixed=arms['fixed75'];mir=arms['mir75'];stop=arms['stop75'];ex=m['explicit']
frac=lambda d:f"{d['complete']}/{d['total']}"
lines=['# Maintaining complete workflows through recurring learning','',
       'Third-phase findings for [experience selection](README.md). Both earlier publications remain preserved.','',
       '## Result','',
       f"Across two prospectively fixed fresh runs, a development-tuned fixed mixture completed **{frac(fixed['all'])}** post-arrival workflows. Damage-aware replay completed **{frac(mir['all'])}** at the same actual update and target-token budgets. Both used 75% replay. The fixed stopping schedule completed **{frac(stop['all'])}** with half as many maintenance updates. The explicit archive reference completed **{ex['success']}/{ex['total']}** with no learning updates.",'']
if fixed['all']==mir['all']:
 lines+=['Damage-aware replay added no complete-task accuracy over the strong fixed mixture in this cohort. Its virtual updates and loss scoring added work. This is a local result for the tested MIR-inspired implementation, not a general result against adaptive replay.','']
elif mir['all']['complete']<fixed['all']['complete']:
 lines+=['The tested damage-aware policy did not improve complete-task accuracy over the strong fixed mixture and required additional selection work. The conclusion is limited to this workload and implementation.','']
else:
 lines+=['Damage-aware replay completed eight more uses overall, but its advantage was concentrated in acquisition: 67/72 versus 55/72. During recurrence it completed fewer uses, 84/96 versus 88/96, and it added 192 virtual updates and 6,144 scoring forwards. This is a measured accuracy–work tradeoff in two runs, not evidence of a general maintenance or compute advantage.','']
if stop['all']['complete']==fixed['all']['complete']==fixed['all']['total']:
 lines+=['Once the first three arrivals had been learned, unconditionally stopping updates preserved complete behavior through the recurring suffix. The saving comes from a fixed stopping schedule on unchanged, already observed rules; it does not require predicting update damage.','']
else:
 lines+=['The stopping control uses a fixed schedule regardless of whether acquisition succeeded. Its accuracy and smaller update count must be considered together; fewer updates alone are not a useful-cost advantage.','']
if a.figure:lines += [f'![Complete workflow trajectories]({a.figure})','']
lines += ['## Fresh complete-task outcomes','', '| Policy | Acquisition prefix | Recurring suffix | All uses | Final | Lost before/after outcomes |','|---|---:|---:|---:|---:|---:|']
labels={'none':'No maintenance','fixed75':'Fixed 75% replay','mir75':'Damage-aware 75% replay','stop75':'Fixed replay, then stop'}
for arm,d in arms.items():lines += [f"| {labels[arm]} | {frac(d['acquisition'])} | {frac(d['recurrence'])} | {frac(d['all'])} | {frac(d['final'])} | {d['pairs']['lost']} |"]
lines += [f"| Explicit archive | 72/72 | 96/96 | {ex['success']}/{ex['total']} | 32/32 | — |",'',
...
for seed in m['seeds']:
 ...
 for arm in ['none','fixed75','mir75','stop75']:
  s=next(s for s in seed['scores'] if s['arm']==arm and s['episode']==6);vals.append(f"{s['success']}/{s['total']}")
 lines += [f"| {seed['seed']} | {initial['success']}/{initial['total']} | "+' | '.join(vals)+' |']
lines += ['', '### Where the fresh policies differ','',
...
for arm in ['fixed75','mir75','stop75']:
 seed=next(s for s in m['seeds'] if s['seed']==107)
 scores=sorted((s for s in seed['scores'] if s['arm']==arm),key=lambda s:s['episode'])
 lines += ['| '+labels[arm]+' | '+' | '.join(f"{s['success']}/{s['total']}" for s in scores)+' |']
lines += ['',
...
for arm,d in arms.items():
 v=d['all'];lines += [f"| {labels[arm]} | {v['resource']}/{v['total']} | {v['preparation']}/{v['total']} | {frac(v)} |"]
lines += ['', 'The selector’s higher complete count accompanies better preparation accuracy, despite slightly lower resource accuracy than fixed replay. Component improvements cannot substitute for joint success.', '', '## Workload and access','',
...
'| Development updates per arrival | Fixed 25% | Fixed 50% | Fixed 75% | Damage-aware 50% | Damage-aware 75% |','|---:|---:|---:|---:|---:|---:|','| 128 | 7/18 | 10/18 | 16/18 | 7/18 | — |','| 256 | — | — | 18/18 | 17/18 | 18/18 |','',
...
for arm,d in arms.items():
 c=d['costs'];u=c['update'];v=c['virtual_update'];s=c['selection_loss']
 lines += [f"| {labels[arm]} | {u['count']} | {v['count']} | {s['count']} | {u['input_tokens']} / {u['target_tokens']} | {c['selection_total_seconds']:.2f} | {u['seconds']:.2f} |"]
lines += ['', '| Policy | Later prompt / output tokens | Later generation seconds |','|---|---:|---:|']
for arm,d in arms.items():
 c=d['costs']['use_generation'];lines += [f"| {labels[arm]} | {c['prompt_tokens']} / {c['completion_tokens']} | {c['seconds']:.2f} |"]
lines += ['', '| Policy | Before-update verification calls | Prompt / output tokens | Generation seconds |','|---|---:|---:|---:|']
for arm,d in arms.items():
 c=d['costs']['verification_generation'];lines += [f"| {labels[arm]} | {c['count']} | {c['prompt_tokens']} / {c['completion_tokens']} | {c['seconds']:.2f} |"]
c=fixed['costs']['construction'];lines += ['',f"The common starting-state construction uses {c['input_tokens']} input tokens and {c['target_tokens']} target tokens in {c['seconds']:.2f} training seconds across both seeds. Verification calls above are separately charged research diagnostics; costs of initial-state evaluation and checkpoint reload generation are retained in the raw run records and whole-run timers.",'']
c=mir['costs'];lines += ['',
 f"Damage-aware scoring additionally processes {c['selection_loss']['input_tokens']} input tokens and {c['selection_loss']['target_tokens']} labeled target tokens; its virtual updates process {c['virtual_update']['input_tokens']} / {c['virtual_update']['target_tokens']}. These are distinct from actual training. Whole-selection time includes cloning, hashing, virtual training, scoring, ranking and restoration, so component timings must not be added again. Token counts are not FLOPs.",'',
 f"The explicit reference stores {ex['unique_archive_rows']} checked records across the two scenarios, with {ex['serialized_archive_bytes_per_run']} serialized archive bytes per run. It performs {ex['retrieval_comparisons']} lookup comparisons and emits {ex['action_tokens']} action tokens. Measured in-memory lookup plus execution totals {ex['seconds']:.6f} seconds; archive construction totals {ex['archive_build_seconds']:.6f} seconds. Those timings exclude persistent-disk access, Python object overhead and real annotation. A saved adapter is {fixed['costs']['adapter_bytes_per_run'][0]} bytes, in addition to the common foundation model.",'',
 f"Actually executing all counterfactual branches and virtual updates costs **{m['actual_execution']['optimizer_updates']} optimizer steps**, plus diagnostics, replay checks and generation. Fresh run timers sum to **{m['actual_execution']['seconds']:.2f} seconds**, with peak MLX allocation **{m['actual_execution']['peak_mlx_bytes']} bytes**. [Resource notes](notes/resource-use.md) record shared-device coordination; these are observed timings, not isolated-hardware latency or a general amortization claim.",'',
