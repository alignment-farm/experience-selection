...
def audit(path, tokenizer):
    ...
    for t,c in zip(tokens,cases['train']):
        assert t['case'] == c
        assert t['prefix'] == tokenizer.apply_chat_template([dict(role='user',content=prompt(c))],
                    tokenize=True,add_generation_prompt=True,enable_thinking=False,return_dict=False)
...
def main():
    ...
    if len(runs)>1:
        ...
        for name in ['protocol.md','followup_experiment.py','followup_task.py','runtime.py','uv.lock']:
            assert len({hashlib.sha256((path/name).read_bytes()).hexdigest() for path in a.runs})==1,name
        if all('followup-fresh-v1' in run['config']['protocol'] for run in runs):
            for devpath in [Path('evidence/followup-development-v1'),Path('evidence/followup-development-v2')]:
                if devpath.exists():
                    devcases=json.loads((devpath/'cases.json').read_text())
                    devtickets={c['ticket'] for suite in devcases.values() for c in suite}
                    assert all(not devtickets&ts for ts in all_tickets)
    totals={}
