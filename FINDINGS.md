# Experience selection: source utility depends on learner state
...
by 18/48 requests, while adding no correct requests to an acquired learner
already at 48/48. A fixed checked-agreement gate matches always-checked training at
96/96 while using 384 subsequent optimizer updates instead of
768. This is a placement and abstention result at a fixed 128-update budget in a small
...
checked 100%, opposite 0%, and base self answers 26/48.
...
|---|---:|---:|
| No update | 30/48 | 48/48 |
| Checked labels, 128 updates | 48/48 | 48/48 |
| Self answers, 128 updates | 24/48 | 48/48 |
| Opposite labels, 128 updates | 0/48 | 0/48 |
| Same checked examples in context, no update | 48/48 | 39/48 |
...
|---|---:|---:|---:|---:|---:|
| 47 | 8/16 | 9/16 | 32/32 | 32/32 | 28.5/32 |
| 53 | 9/16 | 10/16 | 32/32 | 32/32 | 29/32 |
| 59 | 9/16 | 11/16 | 32/32 | 32/32 | 29.5/32 |

The gate and strongest fixed source both score 96/96. Never updating
scores 78/96. Randomly assigning the same one 128-update block
to one of the two starting states gives an exact expected score of
87/96, using measured outcomes for both possible
...
already acquired weights scores 39/48 instead,
so the same-state context comparison totals 87/96. Extra context
...
Its measured gradient-step time sums to 49.71 seconds and
probe generation to 13.95 seconds; always-checked gradient
steps sum to 99.42 seconds. These are sums of
...
The full fresh experiment costs 2688 optimizer updates and
936.6 seconds of recorded run time, including all candidate
branches and checks. Peak recorded MLX allocation is 9.612 GB.
...
checkpoints also score 96/96, using only
192 subsequent updates across the six states, fewer than the
...
starting-state hashes and counts. The fresh runs preserve 1824
response records, of which 96 repeat the initial base
check and are excluded from accuracy denominators. Another 96 source generations
supply self labels and gate probes. All 42 saved fresh adapters
